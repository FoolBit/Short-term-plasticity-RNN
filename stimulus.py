...
class Stimulus:
    ...
    def generate_WM_trial(self, test_mode):
        ...
        test_time_rng.append(range((par['dead_time']+par['fix_time']+2*par['sample_time']+2*par['delay_time'])//par['dt'], \
            (par['dead_time']+par['fix_time']+2*par['sample_time']+2*par['delay_time']+par['test_time'])//par['dt']))
        mask_time_rng.append(range((par['dead_time']+par['fix_time']+2*par['sample_time']+2*par['delay_time'])//par['dt'], \
            (par['dead_time']+par['fix_time']+2*par['sample_time']+2*par['delay_time']+par['mask_duration'])//par['dt']))
        ...
        for t in range(par['batch_size']):
            ...
            cue_stim = np.random.randint(2)
            if test_mode:
                cue_dir = np.random.randint(par['num_motion_dirs'])
            else:
                if trial_info['match'][t,cue_stim] == 1:
                    cue_dir = trial_info['sample'][t,cue_stim]
                else:
                    bad_directions = [trial_info['sample'][t,cue_stim]]
                    possible_stim = np.setdiff1d(list(range(par['num_motion_dirs'])), bad_directions)
                    cue_dir = possible_stim[np.random.randint(len(possible_stim))]
            ...
            # RULE CUE
            trial_info['neural_input'][par['rule_time_rng'][0], t, :] += np.reshape(self.rule_tuning[:,cue_stim],(1,-1))
            # PROBE
